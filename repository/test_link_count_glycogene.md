...
## Endopoint
https://ts.glycosmos.org/sparql
## `glycogene_total_count`小児慢性、指定難病、重複なし合計
```sparql
PREFIX glycan: <http://purl.jp/bio/12/glyco/glycan#>
PREFIX foaf: <http://xmlns.com/foaf/0.1/>
PREFIX dcterms: <http://purl.org/dc/terms/>
...
SELECT
  (COUNT(DISTINCT ?gene_idStr) AS ?count)
  #(GROUP_CONCAT(DISTINCT ?gene_idStr; separator=",") AS ?ids)
...
WHERE {
  VALUES ?glycogene_id {
    {{#each shitei_ncbigene_id_list}}
      <http://glycosmos.org/glycogene/{{this}}>
    {{/each}}
    {{#each shoman_ncbigene_id_list}}
      <http://glycosmos.org/glycogene/{{this}}>
    {{/each}}
  }
...
## Output
```javascript
({result1_shoman,result2_shitei,glycogene_total_count}) => {
  const namedResults = {
    glyco_gene_shoman: result1_shoman,
    glyco_gene_shitei: result2_shitei,
    glyco_gene_total: glycogene_total_count
  };
