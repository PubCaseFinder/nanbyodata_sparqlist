...
({
  json({ total_nando2mondo2gene }) {
    ...
    const total_ncbigene_id_list = Array.from( new Set(nRows.map(r => r.ncbi_id?.value).filter(Boolean)) );
    return {
  ...
  json({ total_nando2mondo2gene, result3_total }) {
    ...
    for (const r of nRows) {
      ...
      out.push({
        ...
        nando_category, // ★追加列
        nando_label_ja: r.nando_label_ja?.value ?? "NA",
